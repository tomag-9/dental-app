...
const CREATE_ENTITY_CONFIG = {
  patient: {
    ...
    initial: () => ({ first_name: '', last_name: '', birth_number: '', insurer: '', phone: '', email: '', address: '' }),
    fields: (workspace) => [
      ...
      { name: 'insurer', label: 'Poisťovňa', type: 'select', placeholder: 'Bez poisťovne', options: (workspace.insurers || []).map((i) => ({ value: String(i.id), label: `${i.code} — ${i.name}` })) },
      { name: 'phone', label: 'Telefón' },
      { name: 'email', label: 'E-mail', type: 'email' },
      { name: 'address', label: 'Adresa' },
    ],
    submit: (form) => window.MolarisAPI.createRecord('/crm/patients/', {
      ...form,
      insurer: form.insurer ? Number(form.insurer) : null,
    }),
  },
  clinic: {
    ...
    initial: () => ({ name: '', ico: '', dic: '', pzs_code: '', email: '', phone: '', street: '', city: '', zip_code: '' }),
    fields: () => [
      { name: 'name', label: 'Názov kliniky', required: true },
      { name: 'ico', label: 'IČO' },
      { name: 'dic', label: 'DIČ' },
      { name: 'pzs_code', label: 'Kód PZS', helpText: 'Kód poskytovateľa zdravotnej starostlivosti, napr. P0599380220.' },
      { name: 'email', label: 'E-mail', type: 'email' },
      { name: 'phone', label: 'Telefón' },
  ...
  doctor: {
    ...
    initial: (workspace) => ({ title_before: 'MUDr.', first_name: '', last_name: '', clinic: workspace.clinics && workspace.clinics[0] ? String(workspace.clinics[0].id) : '', doctor_code: '', registration_number: '', email: '', phone: '' }),
    fields: (workspace) => [
      ...
      { name: 'doctor_code', label: 'Kód lekára', helpText: 'Napr. B06160016.' },
      { name: 'registration_number', label: 'Registračné číslo', helpText: 'Registračné číslo v SKZL.' },
      { name: 'email', label: 'E-mail', type: 'email' },
      { name: 'phone', label: 'Telefón' },
    ],
