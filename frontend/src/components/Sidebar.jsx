...
function Sidebar({ currentPage, onNavigate, user = { name: 'Ján Novák', role: 'admin', initials: 'JN' }, onLogout }) {
  ...
  const [isNarrow, setIsNarrow] = React.useState(() => typeof window !== 'undefined' && window.innerWidth < 1100);
  ...
  // Auto-collapse on narrow viewports without permanently overriding manual expand/collapse.
  const effectiveCollapsed = collapsed || isNarrow;
  // In collapsed mode the rail opens as a flyout on hover without shifting content.
  const col = effectiveCollapsed && !hovered;
  ...
  const navBtn = (id, name, icon, small = false) => {
    ...
    return React.createElement('button', {
      key: id, onClick: () => onNavigate(id), title: col ? name : undefined,
      ...
      style: {
        ...
        padding: col ? '8px 0' : (small ? '6px 10px' : '7px 10px'),
        borderRadius: 6,
        background: active
      ...
      !col && React.createElement('span', null, name)
    );
  };
  ...
  const sectionBtn = (label, icon, expanded, toggle) => React.createElement('button', {
    ...
    style: {
      display: 'flex', alignItems: 'center', gap: col ? 0 : 10,
      justifyContent: col ? 'center' : 'flex-start',
      padding: col ? '8px 0' : '7px 10px', borderRadius: 6,
  ...
  const overlay = effectiveCollapsed && hovered;
  return React.createElement('div', {
    onMouseEnter: () => { if (effectiveCollapsed) setHovered(true); },
    onMouseLeave: () => { if (effectiveCollapsed) setHovered(false); },
    style: {
      width: effectiveCollapsed ? 64 : 224, minHeight: '100%', height: '100%', flexShrink: 0,
      position: 'relative', zIndex: overlay ? 60 : 'auto',
    }
  },
  React.createElement('div', {
    style: {
      width: col ? 64 : 224, height: '100%', background: chrome.bg,
      display: 'flex', flexDirection: 'column', borderRight: `1px solid ${chrome.border}`,
      transition: 'width .2s ease, background .25s', overflow: 'hidden',
      position: overlay ? 'absolute' : 'relative', inset: '0 auto auto 0',
      boxShadow: overlay ? '6px 0 24px rgba(26,35,32,.16)' : 'none',
    }
  },
    // ── Header ──
    React.createElement('div', {
      ...
      col
        ? React.createElement('button', {
          ...
          }, React.createElement(MolarisMark, { size: 24 }))
        : React.createElement(React.Fragment, null,
            React.createElement(isSuperadmin ? MolarisInverse : MolarisLockup, { markSize: 24, wordmarkSize: 17, gap: 9 }),
            React.createElement('button', {
              onClick: () => { setCollapsed(true); setHovered(false); }, title: 'Zbaliť',
    ...
    isSuperadmin && !col && React.createElement('div', {
      ...
      isSuperadmin
        ? React.createElement(React.Fragment, null,
            ...
            React.createElement('div', { key: 'sa-platform', style: { marginTop: 6 } },
              ...
              !col && expandPlatform && subGroup(saPlatform)
            ),
        ...
        : React.createElement(React.Fragment, null,
            ...
            isAdmin && React.createElement('div', { key: 'finance', style: { marginTop: 6 } },
              ...
              !col && expandFinance && subGroup(adminFinance)
            ),
            ...
            isAdmin && React.createElement('div', { key: 'config', style: { marginTop: 6 } },
              ...
              !col && expandConfig && subGroup(adminConfig)
            ),
    ...
    React.createElement('div', { style: { padding: 12, borderTop: `1px solid ${chrome.border}`, flexShrink: 0 } },
      React.createElement('div', { style: { display: 'flex', alignItems: 'center', gap: 9 } },
        ...
        !col && React.createElement('div', { style: { overflow: 'hidden', flex: 1 } },
          React.createElement('p', { title: user.name, style: { fontSize: 12.5, fontWeight: 600, color: chrome.text, margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, user.name),
          React.createElement('p', { style: { fontSize: 10.5, color: chrome.subText, margin: 0 } }, roleLabels[user.role] || 'Používateľ')
        ),
        !col && React.createElement('button', {
